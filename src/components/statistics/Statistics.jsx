...
const fetchStatistics = async (from, to) => {
  try {
    ...
    if (data.result) {
      const mapped = data.result.map((item) => ({
        name: `${item.studentInf.firstName} ${item.studentInf.lastName}`,
  ...
  return (
    <div className="min-h-screen p-2">
      ...
      <div className="grid grid-cols-3 gap-6 h-[60%]">
        ...
        <div className="col-span-2 bg-white rounded-2xl shadow flex flex-col items-center h-full">
          <div className="flex gap-4 mb-0 w-full justify-between items-start relative">
            ...
            <div className="flex flex-col items-center border-none z-10 min-h-[270px] px-2 py-2">
              <DatePicker
                selected={dateFrom}
                onChange={(date) => setDateFrom(date)}
                inline
          ...
          <div className="flex justify-between w-full px-4 py-3">
            <button
              type="button"
              className="text-black bg-[#D9D9D9] text-xl font-zain font-[500] w-[80px] h-[40px] rounded-md shadow"
              onClick={() => {
            ...
            <button
              type="button"
              className="text-white bg-[#97BAA4] text-lg font-zain font-[500] w-[100px] h-[40px] rounded-md shadow"
        ...
        <div className="bg-white col-span-1 rounded-2xl shadow flex flex-col pt-4 h-full self-stretch justify-between ">
          <p className="text-center text-[#55755E] text-xl leading-relaxed font-zain mb-4 p-3 ">
             سابِقُوا إِلَى مَغْفِرَةٍ مِّن رَّبِّكُمْ
             <br />
             وَجَنَّةٍ
             <br />
      ...
      <div className="bg-white rounded-2xl shadow p-4 overflow-x-auto mt-10" dir="rtl">
        <table className="w-full text-right text-gray-700">
          <thead>
            <tr className="border-b ">
              <th className="px-4 py-2 font-ruqaa font-[500] text-shadow text-[#55755E]  text-center">الاسم</th>
              <th className="px-4 py-2 font-ruqaa font-[500] text-shadow text-[#55755E] text-center">عدد صفحات القرآن</th>
              <th className="px-4 py-2 font-ruqaa font-[500] text-shadow text-[#55755E] text-center">عدد صفحات الحديث</th>
              <th className="px-4 py-2 font-ruqaa font-[500] text-shadow text-[#55755E] text-center">عدد مرات الحضور</th>
              <th className="px-4 py-2 font-ruqaa font-[500] text-shadow text-[#55755E] text-center">اخطار البرنامج</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              students.map((s, idx) => (
                <tr key={idx} className="hover:bg-green-50 transition border-b last:border-none ">
                  <td className="px-4 py-2 text-center text-[#55755E]">{s.name}</td>
                  <td className="px-4 py-2 text-center text-[#55755E]">{s.quranPages}</td>
                  <td className="px-4 py-2 text-center text-[#55755E]">{s.reviewPages}</td>
                  <td className="px-4 py-2 text-center text-[#55755E]">{s.attendance}</td>
                  <td className="px-4 py-2 text-center text-[#55755E]">{s.programWarnings}</td>
                </tr>
              ))
            )}
          </tbody>
