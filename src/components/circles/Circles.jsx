...
const handleSubmit = async () => {
  ...
  if (!newBook.pages_num || newBook.pages_num === 0) {
...
const handleDeleteCircle = async (id) => {
  if (!window.confirm("هل أنت متأكد من حذف الحلقة؟")) return;
  ...
  try {
    ...
    console.error("خطأ أثناء الحذف:", err);
    notifyError("حدث خطأ أثناء حذف الحلقة");
    ...
    <div className="flex flex-col gap-6 bg-[#FBFAF8]">
      ...
      <div className="bg-[#97BAA4] rounded-xl p-6 flex justify-between items-center shadow">
        ...
        <div className="text-center  font-zain m-auto">
          ...
          <button className=" border-white border  text-white px-10 py-1 font-ruqaa  rounded-lg shadow hover:bg-[#AFD1BC] transition"  onClick={() => navigate('/createCircle')}>
            إنشاء حلقة
          </button>
        </div>
 ...
 <RiFileList3Line className="text-[#2A603F]  text-center "
            size={23}/>
                </div>
...
<div className="flex flex-row mb-8 justify-start gap-2 w-full">
  <input
    type="number"
    value={newBook.pages_num}
    onChange={(e) =>
      setNewBook({ ...newBook, pages_num: e.target.value })
    }
    className="w-[30%] px-1 py-1 border rounded-md font-zain text-center drop-shadow-black"
  />

  <span className="text-shadow text-right px-1">إدخال عدد الأحاديث</span>
</div>
    ...
    >
      ...
      <div className="flex flex-col items-end max-w-[80%]">
        ...
        <span className="text-xs text-gray-400 mb-3">
          عدد الأحاديث: {book.pages_num}
        </span>
         <span className="text-xs text-gray-400 flex flex-row gap-4">
            <button onClick={() => handleDelete(book.id)}> <RiDeleteBin6Line className="text-[#2A603F] " size={14}/>
       </button>
       <button onClick={() => {
  setNewBook({ name: book.name, pages_num: book.pages_num });
